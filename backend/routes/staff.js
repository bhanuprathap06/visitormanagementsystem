...
router.post('/', async (req, res, next) => {
  try {
    ...
    const [[row]] = await db.query(
      `SELECT s.*,d.department_name,l.location_name FROM STAFF s
       JOIN DEPARTMENT d ON s.department_id=d.department_id
       JOIN LOCATION l ON d.location_id=l.location_id WHERE s.staff_id=?`, [r.insertId]);
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const [[row]] = await db.query(
      `SELECT s.*,d.department_name,l.location_name FROM STAFF s
       JOIN DEPARTMENT d ON s.department_id=d.department_id
       JOIN LOCATION l ON d.location_id=l.location_id WHERE s.staff_id=?`, [req.params.id]);
